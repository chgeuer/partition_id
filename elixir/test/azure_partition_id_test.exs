defmodule AzurePartitionIdTest do
  use ExUnit.Case
  doctest AzurePartitionId
  ...
  describe "determine_partition_id/2 with all test vectors" do
    for {expected_partition, partition_count, partition_key} <- @test_vectors do
      test "returns #{expected_partition} for key #{partition_key} with #{partition_count} partitions" do
        assert AzurePartitionId.determine_partition_id(
                 unquote(partition_key),
                 unquote(partition_count)
               ) == unquote(expected_partition)
      end
    end
  end

  describe "determine_partition_id/2 with opts" do
    test "accepts partition_count as option" do
      assert AzurePartitionId.determine_partition_id(
    ...
    test "raises on missing partition_count option" do
